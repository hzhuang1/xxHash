...
extern uint64_t asvmacc_01(const xxh_u64* XXH_RESTRICT,
...
extern void XXH3_aarch64_sve_consume_stripes(void* XXH_RESTRICT,
				...
				const void* XXH_RESTRICT,
				size_t);
extern void XXH3_aarch64_sve_internal_loop(void* XXH_RESTRICT,
...
/* for SVE-512, and so on */
static xxh_u64 svmacc_01(const xxh_u64* XXH_RESTRICT acc,
                         const xxh_u8* XXH_RESTRICT secret,
                         xxh_u64 start)
{
	svuint64_t xacc, xsec;
	svbool_t p1 = svptrue_pat_b64(SV_VL8);
	svbool_t p2 = svdupq_b64(1, 0);
	svuint64_t mixed_lo, mixed_hi, mixed;
	svuint64_t mul_hi, mul_lo;
	svuint64_t kSwap, swapped;
	uint64_t sum;

	kSwap = sveor_n_u64_z(p1, svindex_u64(0, 1), 1);
	xacc = svld1_u64(p1, (uint64_t *)acc);
	xsec = svld1_u64(p1, (uint64_t *)secret);
...
printf("sum:0x%lx\n", sum);
	sum = sum ^ (sum >> 32);
printf("sum:0x%lx\n", sum);
	//svst1_u64(p1, (uint64_t *)acc, mixed);
	svst1_u64(p1, (uint64_t *)acc, mixed);
	//svst1_u64(p2, (uint64_t *)acc, mixed);
	return sum;
}
#endif	/* __ARM_SVE__ */
...
#else
	XXH128_hash_t product = XXH_mult64to128(acc[0] ^ XXH_readLE64(secret), acc[1] ^ XXH_readLE64(secret+8));
	printf("0x%lx, pl:0x%lx, 0x%lx, ph:0x%lx\n", acc[0] ^ XXH_readLE64(secret), product.low64, acc[1] ^ XXH_readLE64(secret+8), product.high64);
	return product.low64 ^ product.high64;
#endif
}
...
#else
	uint64_t mix;
	mix = XXH3_mix2Accs(acc+2*i, secret + 16*i);
	result64 += mix;
	printf("mix:0x%lx, result64:0x%lx\n", mix, result64);
#endif
    }
...
int svacc_04(void* XXH_RESTRICT acc,
	const void* XXH_RESTRICT input,
	const void* XXH_RESTRICT secret,
	size_t nbStripes)
{
	size_t nbStripesPerBlock;
	size_t secretLimit, secretSize;

	nbStripesPerBlock = 4;
	secretLimit = nbStripesPerBlock * XXH_SECRET_CONSUME_RATE;
	secretSize = secretLimit + XXH_STRIPE_LEN;
	/* load acc into z0 */
	asm volatile (
		"ld1d	z0.d, p7/z, [%[out]]\n\t"
		:
	...
	XXH3_aarch64_sve_init_accum();
	XXH3_aarch64_sve_internal_loop(acc,
					input, 1024,
					secret, secretSize);
}
...
void test_xxh3_internal_loop(size_t nbStripes)
{
	size_t nbStripesPerBlock;
	size_t secretLimit, secretSize;
	...
	XXH3_hashLong_internal_loop(out1, in1, 1024, in2, secretSize,
				XXH3_accumulate_512_scalar,
				XXH3_scrambleAcc_scalar);
	dump_bits("ACC", out1, 512);
}
...
void test_mergeAccs(void)
{
	//xxh_u64 result = 0x55AAAA55;
	xxh_u64 result = 0;

	printf("Test in %s\n", __func__);
	init_buf(in2, 1024);
	set_buf(out1, 0x55, 1024);
	printf("result:0x%llx\n", result);
	result = XXH3_mergeAccs(out1, in2, result);
	printf("mix2accs:0x%llx\n", result);
	result = 0;
	init_buf(in2, 1024);
	set_buf(out1, 0x55, 1024);
	result = svmacc_01(out1, in2, result);
	printf("macc result:0x%lx\n", result);
	dump_bits("ACC", out1, 512);
	result = 0;
	init_buf(in2, 1024);
	set_buf(out1, 0x55, 1024);
	result = asvmacc_01(out1, in1, in2, result);
	printf("asvmacc_01 result:0x%lx\n", result);
	dump_bits("ACC", out1, 512);
}
